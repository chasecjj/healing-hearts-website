...
| Module access is enrollment-gated | All modules require an active enrollment (or admin role) via the `lessons_read` RLS policy. The `is_preview` column is historical and `false` everywhere post-migration 021 — reintroducing `is_preview=true` without a product decision is a regression | Changes to module queries that bypass the enrollment check, or migrations / UI that flip `is_preview=true` |
